...
- 内置固定版本的 Windows x64 llama.cpp CPU 推理框架，按需自启动、空闲卸载并支持任务取消；
- GLM-OCR 模型管理器支持 Hugging Face / 魔搭下载源、断点续传、SHA-256 校验与下载状态恢复；
- OCR 队列接入本地 GLM-OCR 后端，保留原图、原始 JSON 与 Markdown 附件及可恢复任务状态；
- 建立真实 Q8 权重兼容性门禁；CUDA 等硬件加速运行时选择留待后续版本。
